...
def _retire_static_echo_cancel() -> None:
    """Remove the config-file way of loading the echo canceller, where an
    earlier version installed it. A module in the sound config captures the
    microphone around the clock — the OS "mic in use" light never goes out,
    and the one indicator people actually trust becomes noise. The canceller
    now runs as a child process that lives exactly as long as the mic does
    (see audio.py); nothing needs installing for it."""
    ...
    subprocess.run(["systemctl", "--user", "restart",
                   ...
                   capture_output=True)
    time.sleep(2)
    print("Retired the always-on echo-cancel config — the canceller now runs "
          "only while Mantrin holds the mic, so the OS mic light means something.")
